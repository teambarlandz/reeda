...
- App icon: adaptive (foreground/background layers, mipmaps) — **done in
  M7f** (brand green + open-book glyph; vector source
  `android/res/drawable/ic_launcher_foreground.xml`, PNG fallbacks +
  store assets regenerated by `scripts/make_icons.ps1`); screenshots:
  2 phones + 1 tablet, English + localized later (LOCALIZATION.md).
  Screenshots are captured **on device during M7g** (desktop captures
  are not used).
- **Store listing** (drafted M7f): `docs/store/listing.md` (short + full
  description, category, data-safety), feature graphic + 512 px icon in
  `docs/store/`, privacy policy draft in `docs/store/privacy_policy.md`
  (hosted page — see §7).
